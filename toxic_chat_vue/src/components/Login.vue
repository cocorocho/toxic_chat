...
<div class="justify-center items-center w-full shadow rounded-lg bg-white px-6 flex flex-col md:w-1/2 lg:w-1/3 m-auto">
    ...
    <div id="whoobe-h90kl" class="w-full p-2 justify-start flex flex-col">
        ...
        <button value="button" class="px-4 py-2 rounded bg-blue-400 text-white hover:bg-blue-700 my-4 w-full" v-on:click="login">Login</button>
        <h5 class="text-center">Don't have an account? <a class="text-blue-700" href="/register/">Register</a> here.</h5>
    </div>
</div>
</template>
    ...
    methods: {
        async login() {
            ...
            try {
                ...
                vm.$store.commit("setToken", payload)
                this.$router.push("/")
            } catch (e) {
